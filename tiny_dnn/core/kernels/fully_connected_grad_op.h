 ...
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    if (engine == core::backend_t::internal) {
      kernels::fully_connected_op_internal(prev_out, weights, bias, curr_delta,
                                           prev_delta, context.parallelize());
    } else if (engine == core::backend_t::avx) {
      kernels::fully_connected_op_avx(prev_out, weights, bias, curr_delta,
                                      prev_delta, context.parallelize());
    } else {
